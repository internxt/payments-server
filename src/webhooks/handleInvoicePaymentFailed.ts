...
export default async function handleInvoicePaymentFailed(
  ...
  logger.info(`Handling invoice not paid ${invoice.id} for customer ${customer.id} > Suspending object storage account..`);

  await objectStorageService.suspendAccount({ customerId: customer.id });

  logger.info(`Handling invoice not paid ${invoice.id} for customer ${customer.id} > Object storage account suspended..`);
}
